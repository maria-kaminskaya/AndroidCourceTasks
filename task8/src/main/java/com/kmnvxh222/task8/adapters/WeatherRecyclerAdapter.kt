...
class WeatherRecyclerAdapter(private var weatherList: List<WeatherResponse.ListWeather>) :
    ...
    class WeatherViewHolder(itemView: View) :
        ...
        fun bind(weatherResponse: WeatherResponse.ListWeather) {
            val itemIcon = weatherResponse.weather[0].icon

            Glide.with(itemView).load("http://openweathermap.org/img/wn/$itemIcon.png").into(itemView.imageViewWeather)
            ...
            itemView.textViewTemp.text = weatherResponse.main.temp.toString()
        }
